...
def word2vec_basic(log_dir):
  ...
  with open('descriptions.txt', 'r') as file:
    descriptions = file.read().replace('\n', ' ')

  # vocabulary = read_data(filename)
  stop_words = set(stopwords.words('english')) 
  
  remove_punct_dict = dict((ord(punct), None) for punct in string.punctuation)
  vocabulary = descriptions.lower().translate(remove_punct_dict).split()
  vocabulary = [w for w in vocabulary if not (w in stop_words or w.isdigit())]
  ...
  data, count, unused_dictionary, reverse_dictionary = build_dataset(vocabulary)

  vocabulary_size = len(unused_dictionary.keys())
  ...
  valid_examples = np.array([reverse_dictionary.keys()])

  for i, (k, v) in enumerate(unused_dictionary.items()):
    print(k, v)
    if i == 10:
      break
